...
var Path = /** @class */ (function () {
    ...
    Path.prototype.construct = function (segments, offset, length) {
        var path$$1 = Object.create(Object.getPrototypeOf(this));
        path$$1.init(segments, offset, length);
        return path$$1;
    };
...
var ResourcePath = /** @class */ (function (_super) {
     ...
     */
    ResourcePath.fromString = function (path$$1) {
        // NOTE: The client is ignorant of any path segments containing escape
        // sequences (e.g. __id123__) and just passes them through raw (they exist
        // for legacy reasons and should not be used frequently).
        if (path$$1.indexOf('//') >= 0) {
            throw new FirestoreError(Code.INVALID_ARGUMENT, "Invalid path (" + path$$1 + "). Paths must not contain // in them.");
        }
        // We may still have an empty segment at the beginning or end if they had a
        // leading or trailing slash (which we allow).
        var segments = path$$1.split('/').filter(function (segment) { return segment.length > 0; });
...
var FieldPath = /** @class */ (function (_super) {
     ...
     */
    FieldPath.fromServerFormat = function (path$$1) {
        var segments = [];
        var current = '';
        var i = 0;
        var addCurrentSegment = function () {
            if (current.length === 0) {
                throw new FirestoreError(Code.INVALID_ARGUMENT, "Invalid field path (" + path$$1 + "). Paths must not be empty, begin " +
        ...
        while (i < path$$1.length) {
            ...
            if (c === '\\') {
                if (i + 1 === path$$1.length) {
                    throw new FirestoreError(Code.INVALID_ARGUMENT, 'Path has trailing escape character: ' + path$$1);
                }
                var next = path$$1[i + 1];
                if (!(next === '\\' || next === '.' || next === '`')) {
                    throw new FirestoreError(Code.INVALID_ARGUMENT, 'Path has invalid escape sequence: ' + path$$1);
                }
                current += next;
                i += 2;
            }
        ...
        addCurrentSegment();
        if (inBackticks) {
            throw new FirestoreError(Code.INVALID_ARGUMENT, 'Unterminated ` in path: ' + path$$1);
        }
...
var DocumentKey = /** @class */ (function () {
    function DocumentKey(path$$1) {
        this.path = path$$1;
        assert(DocumentKey.isDocumentKey(path$$1), 'Invalid DocumentKey with an odd number of segments: ' +
            path$$1.toArray().join('/'));
    }
    ...
    DocumentKey.isDocumentKey = function (path$$1) {
        return path$$1.length % 2 === 0;
    };
    /**
     ...
     * @return A new instance of DocumentKey
     */
    DocumentKey.fromPathString = function (path$$1) {
        return new DocumentKey(ResourcePath.fromString(path$$1));
    };
...
var Document = /** @class */ (function (_super) {
    ...
    Document.prototype.field = function (path$$1) {
        return this.data.field(path$$1);
    };
    Document.prototype.fieldValue = function (path$$1) {
        var field = this.field(path$$1);
...
var ObjectValue = /** @class */ (function (_super) {
    ...
    ObjectValue.prototype.set = function (path$$1, to) {
        assert(!path$$1.isEmpty(), 'Cannot set field for empty path on ObjectValue');
        if (path$$1.length === 1) {
            return this.setChild(path$$1.firstSegment(), to);
        }
        else {
            var child = this.child(path$$1.firstSegment());
            if (!(child instanceof ObjectValue)) {
                child = ObjectValue.EMPTY;
            }
            var newChild = child.set(path$$1.popFirst(), to);
            return this.setChild(path$$1.firstSegment(), newChild);
        }
    };
    ObjectValue.prototype.delete = function (path$$1) {
        assert(!path$$1.isEmpty(), 'Cannot delete field for empty path on ObjectValue');
        if (path$$1.length === 1) {
            return new ObjectValue(this.internalValue.remove(path$$1.firstSegment()));
        }
        else {
            // nested field
            var child = this.child(path$$1.firstSegment());
            if (child instanceof ObjectValue) {
                var newChild = child.delete(path$$1.popFirst());
                return new ObjectValue(this.internalValue.insert(path$$1.firstSegment(), newChild));
            }
            else {
    ...
    ObjectValue.prototype.contains = function (path$$1) {
        return this.field(path$$1) !== undefined;
    };
    ObjectValue.prototype.field = function (path$$1) {
        assert(!path$$1.isEmpty(), "Can't get field of empty path");
        var field = this;
        path$$1.forEach(function (pathSegment) {
...
var Query = /** @class */ (function () {
    function Query(path$$1, explicitOrderBy, filters, limit, startAt, endAt) {
    ...
    Query.atPath = function (path$$1) {
        return new Query(path$$1);
    };
...
var JsonProtoSerializer = /** @class */ (function () {
    ...
    JsonProtoSerializer.prototype.toResourceName = function (databaseId, path$$1) {
    ...
    JsonProtoSerializer.prototype.toQueryPath = function (path$$1) {
        if (path$$1.length === 0) {
            // If the path is empty, the backend requires we leave off the /documents
            // at the end.
            return this.encodedDatabaseId;
        }
        return this.toResourceName(this.databaseId, path$$1);
    };
    ...
    Object.defineProperty(JsonProtoSerializer.prototype, "encodedDatabaseId", {
        get: function () {
            var path$$1 = new ResourcePath([
                'projects',
                this.databaseId.projectId,
                'databases',
                this.databaseId.database
            ]);
            return path$$1.canonicalString();
        },
        enumerable: true,
        configurable: true
    });
    ...
    JsonProtoSerializer.prototype.isValidResourceName = function (path$$1) {
        // Resource names have at least 4 components (project ID, database ID)
        return (path$$1.length >= 4 &&
            path$$1.get(0) === 'projects' &&
            path$$1.get(2) === 'databases');
    };
    ...
    JsonProtoSerializer.prototype.toQueryTarget = function (query) {
        ...
        if (query.path.isEmpty()) {
            ...
            var path$$1 = query.path;
            assert(path$$1.length % 2 !== 0, 'Document queries with filters are not supported.');
            result.parent = this.toQueryPath(path$$1.popLast());
            result.structuredQuery.from = [{ collectionId: path$$1.lastSegment() }];
        }
    ...
    JsonProtoSerializer.prototype.fromQueryTarget = function (target) {
        ...
        return new Query(path$$1, orderBy, filterBy, limit, startAt, endAt);
    };
    ...
    JsonProtoSerializer.prototype.toFieldPathReference = function (path$$1) {
        return { fieldPath: path$$1.canonicalString() };
    };
    ...
    JsonProtoSerializer.prototype.fromDocumentMask = function (proto) {
        ...
        var fields = paths.map(function (path$$1) { return FieldPath.fromServerFormat(path$$1); });
...
var GrpcConnection = /** @class */ (function () {
    ...
    GrpcConnection.prototype.getRpcCallable = function (rpcName, token) {
        ...
        return function () {
            ...
            return f.apply(void 0, args.concat([metadata]));
        };
    };
...
function loadProtos() {
    ...
    var root = path.resolve(__dirname, "src/protos" || '../protos');
 ...
 */
function encode(path$$1) {
    var result = '';
    for (var i = 0; i < path$$1.length; i++) {
        if (result.length > 0) {
            result = encodeSeparator(result);
        }
        result = encodeSegment(path$$1.get(i), result);
    }
...
function decode(path$$1) {
    ...
    assert(length >= 2, 'Invalid path ' + path$$1);
    if (length === 2) {
        assert(path$$1.charAt(0) === escapeChar && path$$1.charAt(1) === encodedSeparatorChar, 'Non-empty path ' + path$$1 + ' had length 2');
        return ResourcePath.EMPTY_PATH;
    }
    ...
    for (var start = 0; start < length;) {
        ...
        var end = path$$1.indexOf(escapeChar, start);
        if (end < 0 || end > lastReasonableEscapeIndex) {
            fail('Invalid encoded resource path: "' + path$$1 + '"');
        }
        var next = path$$1.charAt(end + 1);
        switch (next) {
            case encodedSeparatorChar:
                var currentPiece = path$$1.substring(start, end);
                var segment = void 0;
                ...
                break;
            case encodedNul:
                segmentBuilder += path$$1.substring(start, end);
                segmentBuilder += '\0';
                break;
            case encodedEscape:
                // The escape character can be used in the output to encode itself.
                segmentBuilder += path$$1.substring(start, end + 1);
                break;
            default:
                fail('Invalid encoded resource path: "' + path$$1 + '"');
        }
...
var DbDocumentMutation = /** @class */ (function () {
     ...
     */
    DbDocumentMutation.prefixForPath = function (userId, path$$1) {
        return [userId, encode(path$$1)];
    };
    /**
     * Creates a full index key of [userId, encodedPath, batchId] for inserting
     * and deleting into the DbDocumentMutations index.
     */
    DbDocumentMutation.key = function (userId, path$$1, batchId) {
        return [userId, encode(path$$1), batchId];
    };
...
var DbNoDocument = /** @class */ (function () {
    function DbNoDocument(path$$1, readTime) {
        this.path = path$$1;
...
var DbUnknownDocument = /** @class */ (function () {
    function DbUnknownDocument(path$$1, version) {
        this.path = path$$1;
        this.version = version;
    }
...
var IndexedDbMutationQueue = /** @class */ (function () {
    ...
    IndexedDbMutationQueue.prototype.getAllMutationBatchesAffectingDocumentKey = function (transaction, documentKey) {
        ...
        return documentMutationsStore(transaction)
            ...
            var path$$1 = decode(encodedPath);
            if (userID !== _this.userId || !documentKey.path.isEqual(path$$1)) {
                control.done();
                return;
            }
    ...
    IndexedDbMutationQueue.prototype.getAllMutationBatchesAffectingQuery = function (transaction, query) {
        ...
        return documentMutationsStore(transaction)
            ...
            var userID = indexKey[0], encodedPath = indexKey[1], batchID = indexKey[2];
            var path$$1 = decode(encodedPath);
            if (userID !== _this.userId || !queryPath.isPrefixOf(path$$1)) {
                control.done();
                return;
            }
            ...
            // ancestor queries.
            if (path$$1.length !== immediateChildrenLength) {
                return;
            }
            uniqueBatchIDs = uniqueBatchIDs.add(batchID);
        })
        ...
        return this.checkEmpty(txn).next(function (empty) {
            ...
            return documentMutationsStore(txn)
                ...
                if (userID !== _this.userId) {
                    ...
                    var path$$1 = decode(key[1]);
                    danglingMutationReferences.push(path$$1);
                }
            })
                .next(function () {
...
var IndexedDbQueryCache = /** @class */ (function () {
    ...
    IndexedDbQueryCache.prototype.addMatchingKeys = function (txn, keys, targetId) {
        ...
        keys.forEach(function (key) {
            var path$$1 = encode(key.path);
            promises.push(store.put(new DbTargetDocument(targetId, path$$1)));
    ...
    IndexedDbQueryCache.prototype.getMatchingKeysForTargetId = function (txn, targetId) {
        ...
        return store
            ...
            var path$$1 = decode(key[1]);
            var docKey = new DocumentKey(path$$1);
    ...
    IndexedDbQueryCache.prototype.containsKey = function (txn, key) {
        var path$$1 = encode(key.path);
        var range = IDBKeyRange.bound([path$$1], [immediateSuccessor(path$$1)], 
        /*lowerOpen=*/ false, 
        /*upperOpen=*/ true);
        var count = 0;
...
var LocalSerializer = /** @class */ (function () {
    ...
    LocalSerializer.prototype.toDbRemoteDocument = function (maybeDoc) {
        if (maybeDoc instanceof Document) {
            ...
            var path$$1 = maybeDoc.key.path.toArray();
            var readTime = this.toDbTimestamp(maybeDoc.version);
            var hasCommittedMutations = maybeDoc.hasCommittedMutations;
            return new DbRemoteDocument(
            /* unknownDocument= */ null, new DbNoDocument(path$$1, readTime), 
            /* document= */ null, hasCommittedMutations);
        }
        else if (maybeDoc instanceof UnknownDocument) {
            var path$$1 = maybeDoc.key.path.toArray();
            var readTime = this.toDbTimestamp(maybeDoc.version);
            return new DbRemoteDocument(new DbUnknownDocument(path$$1, readTime), 
            /* noDocument= */ null, 
            /* document= */ null, 
            /* hasCommittedMutations= */ true);
        }
...
var IndexedDbLruDelegate = /** @class */ (function () {
    ...
    IndexedDbLruDelegate.prototype.forEachOrphanedDocument = function (txn, f) {
        ...
        return store
            ...
            var path$$1 = _b.path, sequenceNumber = _b.sequenceNumber;
            if (targetId === 0) {
 ...
 */
function fromDotSeparatedString(path$$1) {
    var found = path$$1.search(RESERVED);
    if (found >= 0) {
        throw new FirestoreError(Code.INVALID_ARGUMENT, "Invalid field path (" + path$$1 + "). Paths must not contain " +
            "'~', '*', '/', '[', or ']'");
    }
    try {
        return new (FieldPath$1.bind.apply(FieldPath$1, [void 0].concat(path$$1.split('.'))))();
    }
    catch (e) {
        throw new FirestoreError(Code.INVALID_ARGUMENT, "Invalid field path (" + path$$1 + "). Paths must not be empty, " +
...
var ParseContext = /** @class */ (function () {
     ...
     * compromised).
     */
    function ParseContext(dataSource, methodName, path$$1, arrayElement, fieldTransforms, fieldMask) {
...
var UserDataConverter = /** @class */ (function () {
    ...
    UserDataConverter.prototype.parseUpdateData = function (methodName, input) {
        ...
        forEach(input, function (key, value) {
            var path$$1 = fieldPathFromDotSeparatedString(methodName, key);
            var childContext = context.childContextForFieldPath(path$$1);
            value = _this.runPreConverter(value, childContext);
            if (value instanceof DeleteFieldValueImpl) {
                // Add it to the field mask, but don't add anything to updateData.
                fieldMaskPaths.push(path$$1);
            }
            else {
                var parsedValue = _this.parseData(value, childContext);
                if (parsedValue != null) {
                    fieldMaskPaths.push(path$$1);
                    updateData = updateData.set(path$$1, parsedValue);
                }
            }
        });
 ...
 */
function fieldPathFromArgument(methodName, path$$1) {
    if (path$$1 instanceof FieldPath$1) {
        return path$$1._internalPath;
    }
    else if (typeof path$$1 === 'string') {
        return fieldPathFromDotSeparatedString(methodName, path$$1);
    }
    else {
 ...
 * on dots.
 */
function fieldPathFromDotSeparatedString(methodName, path$$1) {
    try {
        return fromDotSeparatedString(path$$1)._internalPath;
    }
    catch (e) {
        var message = errorMessage(e);
        throw new FirestoreError(Code.INVALID_ARGUMENT, "Function " + methodName + "() called with invalid data. " + message);
...
var DocumentReference = /** @class */ (function () {
    ...
    DocumentReference.forPath = function (path$$1, firestore) {
        if (path$$1.length % 2 !== 0) {
            throw new FirestoreError(Code.INVALID_ARGUMENT, 'Invalid document reference. Document ' +
                'references must have an even number of segments, but ' +
                (path$$1.canonicalString() + " has " + path$$1.length));
        }
        return new DocumentReference(new DocumentKey(path$$1), firestore);
    };
    ...
    DocumentReference.prototype.collection = function (pathString) {
        ...
        var path$$1 = ResourcePath.fromString(pathString);
        return new CollectionReference(this._key.path.child(path$$1), this.firestore);
    };
    ...
    Query$$1.prototype.where = function (field, opStr, value) {
        ...
        if (fieldPath.isKeyField()) {
            ...
            if (typeof value === 'string') {
                ...
                var path$$1 = this._query.path.child(new ResourcePath([value]));
                assert(path$$1.length % 2 === 0, 'Path should be a document key');
                fieldValue = new RefValue(this.firestore._databaseId, new DocumentKey(path$$1));
            }
...
var CollectionReference = /** @class */ (function (_super) {
    ...
    function CollectionReference(path$$1, firestore) {
        var _this = _super.call(this, Query.atPath(path$$1), firestore) || this;
        if (path$$1.length % 2 !== 1) {
    ...
    CollectionReference.prototype.doc = function (pathString) {
        ...
        var path$$1 = ResourcePath.fromString(pathString);
        return DocumentReference.forPath(this._query.path.child(path$$1), this.firestore);
    };
